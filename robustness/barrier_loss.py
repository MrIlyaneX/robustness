...
def per_sample_margin_loss(logits, labels, delta):
    ...
    highest_incorrect_logit = logits.masked_fill(F.one_hot(labels, num_classes=logits.shape[1]).bool(), -1e9).max(1).values
    margins = correct_label_logit - highest_incorrect_logit
    g_theta = delta - margins
    return g_theta


def logarithmic_barrier_loss(logits, labels, delta, mu):
    '''
    ...
    barrier_loss = torch.inf
    if torch.any(g_theta < 0):
        barrier_loss = -mu * torch.log(-g_theta).mean()
