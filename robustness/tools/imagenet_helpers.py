...
class ImageNetHierarchy:
    ...
    def __init__(self, ds_path, ds_info_path):
        ...
        del_nodes = [
            wnid
            for wnid in self.tree
            if (
                self.tree[wnid].descendant_count_in == 0
                and self.tree[wnid].class_num == -1
            )
        ]
        for d in del_nodes:
        ...
        assert all(
            [k.descendant_count_in > 0 or k.class_num != -1 for k in self.tree.values()]
        )

        self.wnid_sorted = sorted(
            sorted(
                [
                    (k, v.descendant_count_in, len(v.descendants_all))
    ...
    def get_descendants(self, node_wnid, in_imagenet=False):
        ...
        if in_imagenet:
            return set(
                [
                    self.wnid_to_num[ww]
                    for ww in self.tree[node_wnid].descendants_all
    ...
    def get_superclasses(
        self, n_superclasses, ancestor_wnid=None, superclass_lowest=None, balanced=True
    ):
        """
        ...
        assert superclass_lowest is None or not any(
            [
                self.is_ancestor(s1, s2)
                for s1, s2 in product(superclass_lowest, superclass_lowest)
            ]
        )
        ...
        class_ranges, label_map = self.get_subclasses(
            superclass_wnid, balanced=balanced
        )
        ...
        for ii, w in enumerate(superclass_wnid):
            ...
            if balanced and len(descendants) > ndesc_min:
                descendants = set(
                    [
                        dd
                        for ii, dd in enumerate(sorted(list(descendants)))
                        if ii < ndesc_min
                    ]
                )
